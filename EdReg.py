...
class EdReg():
    ...
    def energy_transfer(self):
        ...
        for i in range(len(self.time_list)):
            try:
                if self.time_list[i] >= now:
                    ...
                    mode_flag = 0
                    break
                elif self.time_list[i] <= now < self.time_list[i+1] : 
                    self.ET_last = self.energytransfer_list[i-1] # (容量)
                    self.ET_now = self.energytransfer_list[i]    # (容量)
                    self.ET_next = self.energytransfer_list[i+1] # (容量)
                    self.ET_time = self.time_list[i] # 排程時段(時間)
        ...
        return self.ET_last, self.ET_now, self.ET_next, mode_flag
